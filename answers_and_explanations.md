...
- 03-5. [중재자 (Mediator)](#03-5-중재자-mediator)
- 03-6. [메멘토 (Memento)](#03-6-메멘토-memento)
- 03-7. [옵저버 (Observer)](#03-7-옵저버-observer)
- 03-8. [상태 (State)](#03-8-상태-state)
- 03-9. [전략 (Strategy)](#03-9-전략-strategy)
- 03-10. [템플릿 메서드 (Template Method)](#03-10-템플릿-메서드-template-method)
- 03-11. [방문자 (Visitor)](#03-11-방문자-visitor)
...
<a id="03-5-중재자-mediator"></a>

### 03-5. 중재자 (Mediator)

#### 문제 1
**문제**: 중재자 패턴의 핵심 의도에 가장 가까운 것은?
...
**정답**: 서로 얽힌 객체 간 상호작용을 중앙 중재자에게 위임해 결합도를 낮춘다.

**해설**: Mediator는 Colleague들이 서로 직접 의존하지 않고 중앙 조정자에게 상호작용을 위임하도록 해 결합도를 낮춥니다. 인터페이스 변환은 어댑터, 동적 책임 추가는 데코레이터, 서브시스템 단순화는 퍼사드의 의도입니다.

#### 문제 2
**문제**: 다음 중 스프링에서 "중재자" 성격이 가장 뚜렷한 것은?
...
**정답**: ApplicationEventPublisher와 이벤트 리스너

**해설**: 이벤트 퍼블리셔는 발행자와 리스너를 직접 연결하지 않고 중앙 허브가 이벤트를 전달합니다. 이는 전형적인 중재자 구조입니다. 커넥션 풀은 리소스 관리, @Transactional은 프록시/AOP, Bean Validation은 제약 검사에 초점이 있습니다.

#### 문제 3 (복수 응답)
**문제**: 중재자 패턴 적용을 고려하기 좋은 상황을 모두 고르시오.
...
**정답**: 다수 컴포넌트의 상호작용 규칙이 빈번히 바뀌고 복잡할 때, 발신자-수신자를 간접 연결하는 메시지 브로커 사용, UI 폼 컴포넌트 상태 전이를 중앙 조정

**해설**:
- 상호작용 규칙 복잡/빈번 변경: 규칙을 Mediator에 집중해 변경 파급을 줄일 수 있습니다.
- 메시지 브로커: 발신자/수신자를 허브가 중재하는 구조로 Mediator의 철학과 일치합니다.
- UI 폼 조정: 입력/버튼/에러표시 등 컴포넌트를 중앙에서 오케스트레이션하기 좋습니다.
- 트리 구조 통합은 컴포지트의 관심사이며, 단순 1:1 호출만 있는 경우엔 Mediator가 과한 설계입니다.


---

<a id="03-6-메멘토-memento"></a>

### 03-6. 메멘토 (Memento)

#### 문제 1
**문제**: 메멘토 패턴의 핵심 의도에 가장 가까운 것은?
...
**정답**: 내부 상태를 캡슐화한 스냅샷으로 저장하고 필요 시 복구한다.

**해설**: 메멘토는 Originator의 내부 상태를 외부에 노출하지 않고(Memento로 캡슐화) 저장했다가, 필요 시 해당 스냅샷으로 복구합니다. 동적 책임 추가는 데코레이터, 인터페이스 변환은 어댑터, 요청 캡슐화는 커맨드의 의도입니다.

#### 문제 2
**문제**: 다음 중 메멘토 패턴의 올바른 분리 책임 설명으로 가장 적절한 것은?
...
**정답**: Originator는 메멘토를 만들고 복구하며, Caretaker는 그것을 보관한다.

**해설**: 메멘토의 핵심은 캡슐화 유지입니다. 메멘토 내부는 외부에서 수정하거나 들여다보지 못해야 하며, 복구 행위는 Originator만 수행합니다. Caretaker는 보관·관리만 담당합니다.

#### 문제 3 (복수 응답)
**문제**: 메멘토(또는 유사 개념)를 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: 텍스트 편집기의 Undo/Redo 구현, 데이터베이스 트랜잭션 세이브포인트 사용, 이벤트 소싱 시스템의 상태 스냅샷 저장

**해설**:
- Undo/Redo: 사용자 편집 상태를 스냅샷으로 저장·복구하는 전형적 사례입니다.
- 트랜잭션 세이브포인트: 특정 지점으로 롤백 가능한 점에서 메멘토적 사고와 유사합니다.
- 이벤트 소싱 스냅샷: 많은 이벤트 재생을 줄이기 위해 상태 스냅샷을 저장·복구합니다.
- 인터페이스 변환은 어댑터, 프록시를 통한 접근 제어·로깅은 프록시 패턴의 관심사입니다.


---

<a id="03-7-옵저버-observer"></a>

### 03-7. 옵저버 (Observer)

#### 문제 1
**문제**: 옵저버 패턴의 핵심 의도로 가장 적절한 것은?
...
**정답**: 어떤 객체의 상태 변화가 관련 객체들에게 자동으로 통지되도록 일대다 의존을 정의한다.

**해설**: 옵저버는 Subject(발행자)의 상태 변화가 등록된 Observer(구독자)들에게 브로드캐스트되도록 합니다. 단계별 생성은 빌더, 인터페이스 변환은 어댑터, 동적 책임 추가는 데코레이터의 의도입니다.

#### 문제 2
**문제**: 다음 중 스프링에서 옵저버 패턴의 적용으로 가장 적절한 것은?
...
**정답**: @EventListener를 통한 ApplicationEvent 구독 처리

**해설**: 스프링 이벤트 퍼블리셔와 @EventListener는 전형적인 발행-구독(옵저버) 모델입니다. RestTemplate 호출은 단순 HTTP 클라이언트, @Transactional은 프록시/AOP, 인터셉터 체인은 책임 연쇄(CoR)에 가깝습니다.

#### 문제 3 (복수 응답)
**문제**: 옵저버 패턴을 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: 주문 생성 시 이메일 알림·포인트 적립·통계 집계 분리 처리, 웹소켓/SSE 실시간 이벤트 전파, 엔티티 수명주기 이벤트 반응

**해설**:
- 도메인 이벤트 발행 후 여러 리스너가 독립적으로 반응하면 결합도가 낮아지고 확장성이 좋아집니다.
- 실시간 알림 전파(웹소켓/SSE)는 다수 구독자로의 통지 모델과 잘 맞습니다.
- JPA/Hibernate 엔티티 리스너는 수명주기 이벤트에 반응하는 옵저버의 대표적 사례입니다.
- 인터페이스 변환은 어댑터의 관심사이며, 복잡 규칙의 중앙 조정은 중재자(Mediator)가 더 적합합니다.


<a id="03-8-상태-state"></a>

### 03-8. 상태 (State)

#### 문제 1
**문제**: 상태 패턴의 핵심 의도에 가장 가까운 것은?
...
**정답**: 객체의 내부 상태에 따라 행동이 바뀌도록 상태별 클래스로 캡슐화하고 전이를 관리한다.

**해설**: 상태(State)는 if-else/switch 분기를 상태 객체 교체로 대체하여, 상태별 규칙과 행동을 각 상태 클래스에 캡슐화합니다. 동적 책임 추가는 데코레이터, 인터페이스 변환은 어댑터, 서브시스템 단순화는 퍼사드의 의도입니다.

#### 문제 2
**문제**: 다음 중 스프링에서 상태 패턴(또는 상태 머신)의 적용으로 가장 적절한 것은?
...
**정답**: Spring Statemachine으로 주문 상태 전이와 가드/액션 구성

**해설**: Spring Statemachine은 상태/이벤트/전이/가드/액션을 선언적으로 정의하는 상태 머신 프레임워크입니다. @Transactional은 프록시/AOP, 인터셉터 체인은 책임 연쇄(CoR), RestTemplate 호출은 HTTP 클라이언트 사용입니다.

#### 문제 3 (복수 응답)
**문제**: 상태 패턴을 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: 주문/결제/배송처럼 명확한 전이 규칙과 행동 변화, 문서 승인 워크플로우 명시, 상태에 따른 기능/버튼 활성화 변화

**해설**:
- 전이 규칙과 행동 변화가 명확한 도메인(주문/결제/배송)은 상태별 행동 캡슐화와 전이 관리가 유익합니다.
- 문서 승인은 상태(초안·제출·승인/반려)에 따라 허용 작업이 달라지는 전형적 상태 도메인입니다.
- UI/권한에서 상태별 기능 활성화/비활성화를 관리하기에도 상태 패턴이 적합합니다.
- 트리 구조 통합은 컴포지트의 관심사이며, 인터페이스 불일치 해결은 어댑터의 관심사입니다.


<a id="03-9-전략-strategy"></a>

### 03-9. 전략 (Strategy)

#### 문제 1
**문제**: 전략 패턴의 핵심 의도로 가장 적절한 것은?
...
**정답**: 알고리즘군을 정의하고 캡슐화하여 상호 교환 가능하게 만든다.

**해설**: Strategy는 공통 계약(Strategy)에 따라 다양한 알고리즘(ConcreteStrategy)을 캡슐화해 Context에서 교체 가능하게 하는 것이 핵심입니다. 요청 캡슐화는 커맨드, 인터페이스 변환은 어댑터, 상태 스냅샷은 메멘토의 의도입니다.

#### 문제 2
**문제**: 다음 중 스프링에서 "전략" 교체의 대표적 사례로 가장 적절한 것은?
...
**정답**: PasswordEncoder의 구현 교체 및 DelegatingPasswordEncoder 위임

**해설**: PasswordEncoder는 공통 인터페이스 뒤에서 BCrypt/PBKDF2 등 전략을 교체합니다. DelegatingPasswordEncoder는 저장된 해시의 {id}를 읽어 적절한 인코더로 위임합니다. @Transactional은 프록시(AOP), 이벤트 브로드캐스트는 옵저버, 인터셉터 체인은 책임 연쇄에 가깝습니다.

#### 문제 3 (복수 응답)
**문제**: 전략 패턴을 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: 사용자 등급/프로모션에 따른 할인 정책 적용, Content-Type/Accept에 따른 직렬화 방식 선택, 정렬 기준에 따른 Comparator 교체

**해설**:
- 할인 정책/요금 정책처럼 조건에 따라 알고리즘을 바꿔야 하는 곳은 전형적 Strategy 적용처입니다.
- HttpMessageConverter 선택(미디어 타입 기반)이나 직렬화 포맷 선택은 전략군 중 하나를 상황에 맞게 고르는 형태입니다.
- Comparator 교체는 동일 계약(compare) 뒤에서 다양한 정렬 기준 전략을 바꾸는 사례입니다.
- 트리 구조 통합은 컴포지트, DSL 해석은 인터프리터의 관심사입니다.


---

<a id="03-10-템플릿-메서드-template-method"></a>

### 03-10. 템플릿 메서드 (Template Method)

#### 문제 1
**문제**: 템플릿 메서드 패턴의 핵심 의도에 가장 가까운 것은?
...
**정답**: 알고리즘의 골격을 상위 클래스에 정의하고, 일부 단계를 하위 클래스가 재정의한다.

**해설**: 템플릿 메서드는 상위 클래스의 final 템플릿에서 전체 절차를 고정하고, 변하는 지점(훅)을 하위 클래스가 오버라이드합니다. 요청 캡슐화는 커맨드, 인터페이스 변환은 어댑터, 상태 스냅샷은 메멘토의 의도입니다.

#### 문제 2
**문제**: 다음 중 스프링에서 "템플릿/콜백"이 제공하는 장점으로 가장 적절한 것은?
...
**정답**: 콜백을 통해 자원 관리/예외 변환 같은 공통 흐름을 템플릿이 맡고, 가변 로직만 주입해 실행한다.

**해설**: JdbcTemplate 등은 커넥션 획득/해제, 예외 변환, 트랜잭션 경계를 템플릿이 담당하고, 사용자 로직은 RowMapper/PreparedStatementCreator 같은 콜백으로 주입합니다. 큐잉/재시도는 커맨드, 상태 캡슐화는 상태 패턴, 트리 구조는 컴포지트의 관심사입니다.

#### 문제 3 (복수 응답)
**문제**: 템플릿 메서드(또는 템플릿/콜백)를 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: DB 접근 공통 처리+행 매핑만 다름, 동일 절차 표준화+일부 단계만 변경, 외부 API 공통 흐름 동일·세부만 다름

**해설**:
- DB 접근은 보일러플레이트가 반복되므로 템플릿이 자원/예외를 책임지고, 가변 부분(RowMapper 등)만 콜백으로 분리하면 적합합니다.
- 전처리→핵심→후처리의 공통 절차가 많을수록 템플릿 골격을 두고 단계별 훅만 바꾸는 구조가 유리합니다.
- 외부 API 호출도 재시도/로깅/오류 처리 같은 공통 흐름을 템플릿화하고, 요청/응답 포맷 차이는 콜백/전략으로 주입하면 효과적입니다.
- 알고리즘 교체는 전략(Strategy), DSL 해석은 인터프리터(Interpreter)의 영역입니다.


---

<a id="03-11-방문자-visitor"></a>

### 03-11. 방문자 (Visitor)

#### 문제 1
**문제**: 방문자 패턴의 핵심 의도에 가장 가까운 것은?
...
**정답**: 구조를 바꾸지 않고 새로운 연산을 요소 계층 밖(방문자)으로 분리해 추가한다.

**해설**: 방문자는 Element 구조를 변경하지 않고도 새로운 연산을 Visitor로 외부화해 추가할 수 있게 합니다. 스냅샷 저장/복구는 메멘토, 인터페이스 변환은 어댑터, 동적 책임 추가는 데코레이터의 의도입니다.

#### 문제 2
**문제**: 다음 중 자바/스프링에서 "방문자" 개념과 가장 관련이 깊은 것은?
...
**정답**: java.nio.file.FileVisitor를 이용한 디렉터리 순회

**해설**: FileVisitor는 디렉터리 트리를 방문하며 노드 타입별 콜백을 실행하는 전형적인 방문자 모델입니다. @Transactional은 프록시/AOP, HttpMessageConverter는 전략군 선택에 가깝고, RestTemplate 호출은 방문자와 직접 관련이 없습니다.

#### 문제 3 (복수 응답)
**문제**: 방문자 패턴을 적용하기 좋은 상황을 모두 고르시오.
...
**정답**: 메뉴/카테고리 트리 권한 필터링/렌더링, AST/표현식의 다수 연산 추가, 디렉터리 트리를 방문하며 다양한 작업 수행

**해설**:
- 계층 구조(메뉴/카테고리) 위에 여러 연산을 번갈아 적용해야 하면, 구조를 고정하고 방문자만 바꾸는 방식이 적합합니다.
- AST/표현식 처리에서는 해석/검증/최적화 등 서로 다른 연산을 방문자로 추가하기 용이합니다.
- 디렉터리 트리 작업에서도 방문자 교체로 수집/복사/인덱싱 등 동작을 바꾸기 쉽습니다.
- "트리 구조를 동일 인터페이스로 다룬다"는 요구는 컴포지트의 관심사이며, 단순 1:1 호출만 있는 흐름에 방문자를 도입하는 것은 과한 설계입니다.
